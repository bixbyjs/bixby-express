 ...
 * @return {Function}
 */
exports = module.exports = function(keypair, settings, logger) {
    ...
    if (cluster.isMaster && config.cluster) {
      ...
      if (config.secure) {
        ...
        function server(err, options)
        {
          ...
          logger.debug('HTTPS using options',options);
          self.httpsServer = https.createServer(options, self);
          self.httpsServer.listen(port, address, function() {
 ...
 */
exports['@require'] = [ 'akeypair', 'settings', 'logger' ];
